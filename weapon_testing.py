...
def front_weapons(*weapons):
    ...
    if weapons[0] == '': #check for empty input
        ...
        for weapon in weapons:
            ...
            front_arc_weapons_names.append(name[0])
        total_front_arc_mass = sum(front_arc_mass)
        total_front_arc_pv = sum(front_arc_pv)
        total_front_arc_max_dmg = sum(front_arc_max_dmg)
    return front_arc_weapons_names, total_front_arc_mass, total_front_arc_pv, total_front_arc_max_dmg

#UI
weapon_selection = True
while weapon_selection == True:
    arc = input(f"\nSelect arc (1-Front, 2-Rear, 3-Right, 4-Left) or E to exit: ").upper()
    match arc:
        case "1":
